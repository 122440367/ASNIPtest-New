...
**Linux / macOS**
```bash
curl -fsSL https://raw.githubusercontent.com/122440367/ASNIPtest-New/refs/heads/main/install.sh | bash
cmtjd
```
...
运行完成后会生成带时间戳的 CSV，并刷新固定的最新结果文件：

```
📥 下载链接 (常驻):
最新结果: http://1.2.3.4:8899/output_latest.csv
本次结果: http://1.2.3.4:8899/output_AS209242_20260617_120000.csv
...
> 下载链接自动检测公网 IP（ipify → ip.sb → 宿主机网关三重兜底），Docker/NAT 环境下也能正常工作。服务会常驻运行，适合 cron/systemd 无人值守任务。
